...
![dsh-token-dashboard 截图](screenshot.png)
...
- **浏览器端（`client.js`）** 是完全自包含的 bundle（不依赖 React、不用插槽、不用主题工具库 —— 只有 `fetch` + DOM + SVG）。它渲染一个固定定位的小窗，可折叠为**只读的迷你胶囊**（展示当前时间窗口内全部会话汇总的 6 项核心指标）、可在页面上拖动、可沿**四边与四角手动拉伸尺寸**，展开后显示汇总数值、图表、时间窗口筛选器与会话选择器。
...
| 余额拉取 | 固定每 30 分钟 | 对齐本地时间整点/半点（`:00` / `:30`）拉取官方余额，采样时刻整齐；启动时立即拉取一次 |
...
- **总消耗（汇总）** —— 在所选时间窗口内，把**所有会话合并**为一组数值卡片与图表：
  ...
  - 第一个图表的数据源**可点击切换**：输出 / 输入 / 缓存读取 / 总消耗 / 调用次数；第二个图表固定为每小时总消耗（**API 整体消耗**：uncached 输入 + 缓存读取 + 缓存写入 + 输出）。未计费环境下不再按计费口径折算，直接统计整体流量。
...
- **模型** —— **按提供商分组**展示：每个提供商一段分组标题（组名 + 组内模型数 + 组内总消耗 + 占窗口比例），组内每个「提供商/模型」组合一张卡片，显示**哪个模型消耗了多少 token**：输入（uncached）/ 缓存读取 / 输出、该模型的 API 调用次数，以及一个带缓存命中率的总消耗格（**API 整体消耗**：uncached 输入 + 缓存读取 + 缓存写入 + 输出）和一条占窗口总消耗比例的条形图。卡片按总消耗降序排列。面板顶部有**提供商筛选下拉**（默认"全部提供商"，选择后只显示该提供商的组与卡片，选择会记住）。提供商/模型组合读取自每条 `assistant/message` 的 `data.message.source`（缺失时归入 未知|未知），因此混用多模型的对话能把每个 token 都归因到真正生成它的模型。
...
**1小时**范围是特殊的：它以**每分钟**粒度渲染趋势图（连续的 60 分钟轴，空缺补零），便于逐分钟观察最近一段活动。小窗约每 2.5 秒轮询一次，因此面板打开时大约每分钟会出现一个新的分钟数据点。**1月/全部**范围则以**每天**粒度渲染（每天一个点，由当天的小时桶折叠而成），趋势点单位与悬浮时间随之显示「天」；1天/1周范围保持小时级（显示「时」）。分钟级桶以约 25 小时的滚动缓冲保存在内存中；小时桶常驻，天级序列由小时桶折叠而来。
...
小窗默认宽约 440px（3 列数值网格），可抓住**四边或四角**手动拉伸：宽度 300px 起、高度 110px 起，均以视口为上限；折叠状态下只允许横向拉伸（高度由头部一行决定）。自定义尺寸保存在 `dsh-token-dashboard:size`（`{w, h}`）中，与位置一样跨刷新/重启保留；拉伸会把面板从默认角落停靠切换为显式定位（与拖拽一致）。

**折叠态 = 迷你胶囊**：收起时整个面板隐藏，只剩一个约 36px 高的紧凑胶囊，固定显示当前时间窗口内**全部会话汇总**的 6 项核心指标（与当前视图无关）：总消耗（**API 整体消耗**，含缓存读取）、输入、输出、缓存读取、总命中率、API 调用次数，每项一个带指标色的小 chip，悬停可看完整含义。胶囊同样可拖拽移动，点击即展开完整面板；连接失败时胶囊隐藏数字、显示红色「连接失败」并附错误 tooltip。连接/回填状态标记（`连接中…` / `回填中…` / `已回填`）只在展开时可见。
...
- `GET /token-dashboard/api[?range=all|1h|1d|7d|30d]` → `{ ok, now, range, activeId, config, count, backfilled, backfillError, totals, series, models[], balance, sessions[] }`
  ...
  - `models[]` —— 窗口内的每模型数据 `{ provider, model, totals, hitPct, sharePct }`：
    ...
    - `sharePct` —— 占窗口总消耗（**API 整体消耗**：uncached + 缓存读取 + 缓存写入 + 输出）的比例，百分比 0-100；条目按整体消耗降序排列
