...
**折叠态 = 迷你胶囊**：收起时整个面板隐藏，只剩一个约 36px 高的紧凑胶囊，固定显示当前时间窗口内**全部会话汇总**的 6 项核心指标（与当前视图无关）：总消耗（uncached 输入 + 输出）、输入、输出、缓存读取、总命中率、API 调用次数，每项一个带指标色的小 chip，悬停可看完整含义。胶囊同样可拖拽移动，点击即展开完整面板；连接失败时胶囊隐藏数字、显示红色「连接失败」并附错误 tooltip。连接/回填状态标记（`连接中…` / `回填中…` / `已回填`）只在展开时可见。
